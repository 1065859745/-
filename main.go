...
var reg, port, checkIp, checkIpMsg, startMsg = regexp.MustCompile(`@\d{11}`), flag.String("p", "5001", "Transfer server prot"), flag.String(`i`, ``, "Check connect status by remote ip"), flag.String(`m`, `监控异常，请检查连接`, "Send mssages when connect faild"), flag.String(`s`, ``, `Send the messages when connect to dingtalk`)
...
func main() {
	...
	// 如果使用vpn连接，检查vpn状态
	if *checkIp != `` {
		go checkConn(*checkIp)
	}
	// 测试钉钉url
	sendText(*startMsg, reg.FindAllString(*startMsg, -1), false)
...
func checkErr(e error) {
	if e != nil {
		sendText(*checkIpMsg, reg.FindAllString(*checkIpMsg, -1), false)
		log.Fatal(e)
		wg.Done()
	}
}
